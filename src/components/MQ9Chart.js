...
const MQ9Chart = () => {
    const [mq9AnalogicoData, setMq9AnalogicoData] = useState([]);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:5000/api/mq9'); // Asegúrate de que esta ruta sea correcta
                const { mq9_analogico } = response.data;

                setMq9AnalogicoData(prev => [...prev, mq9_analogico]);
            } catch (error) {
